...
#         elif self.ctrl_status == 'wait_lowest_point':
#             # 抵達目標最低點附近 → 排程射擊
#             if abs(self.red_x - self.lowest_x) < 2 and abs(self.red_y - self.lowest_y) < 30 and self.cur_det_ns:
...
class Strategy(API):
    ...
    def initial(self):
        self.x_points = []
        self.y_points = []
        self.x_points_temp = 0
        self.y_points_temp = 0
        self.orbit_trail = []
        self.first_point = False
        self.ctrl_status = 'find_period'
        self.datum_mark_x = 0
    ...
    def run(self):
        ...
        if self.ctrl_status == 'find_period':
            ...
            # 抵達目標最低點附近 → 排程射擊
            if abs(self.red_x - self.lowest_x) < 1 and abs(self.red_y - self.lowest_y) < 30 and hasattr(self, 'period_s'):
                fire_delay_s = self.period_s - SHOOT_DELAY
                if fire_delay_s < 0:
                    fire_delay_s = fire_delay_s + self.period_s
                self.get_logger().info(f"at lowest y, schedule shoot in {fire_delay_s:.3f}s")
