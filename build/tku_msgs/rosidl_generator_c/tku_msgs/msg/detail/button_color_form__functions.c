// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from tku_msgs:msg/ButtonColorForm.idl
// generated code does not contain a copyright notice
#include "tku_msgs/msg/detail/button_color_form__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
tku_msgs__msg__ButtonColorForm__init(tku_msgs__msg__ButtonColorForm * msg)
{
  if (!msg) {
    return false;
  }
  // buildingmodel
  return true;
}

void
tku_msgs__msg__ButtonColorForm__fini(tku_msgs__msg__ButtonColorForm * msg)
{
  if (!msg) {
    return;
  }
  // buildingmodel
}

bool
tku_msgs__msg__ButtonColorForm__are_equal(const tku_msgs__msg__ButtonColorForm * lhs, const tku_msgs__msg__ButtonColorForm * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // buildingmodel
  if (lhs->buildingmodel != rhs->buildingmodel) {
    return false;
  }
  return true;
}

bool
tku_msgs__msg__ButtonColorForm__copy(
  const tku_msgs__msg__ButtonColorForm * input,
  tku_msgs__msg__ButtonColorForm * output)
{
  if (!input || !output) {
    return false;
  }
  // buildingmodel
  output->buildingmodel = input->buildingmodel;
  return true;
}

tku_msgs__msg__ButtonColorForm *
tku_msgs__msg__ButtonColorForm__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tku_msgs__msg__ButtonColorForm * msg = (tku_msgs__msg__ButtonColorForm *)allocator.allocate(sizeof(tku_msgs__msg__ButtonColorForm), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tku_msgs__msg__ButtonColorForm));
  bool success = tku_msgs__msg__ButtonColorForm__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tku_msgs__msg__ButtonColorForm__destroy(tku_msgs__msg__ButtonColorForm * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tku_msgs__msg__ButtonColorForm__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tku_msgs__msg__ButtonColorForm__Sequence__init(tku_msgs__msg__ButtonColorForm__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tku_msgs__msg__ButtonColorForm * data = NULL;

  if (size) {
    data = (tku_msgs__msg__ButtonColorForm *)allocator.zero_allocate(size, sizeof(tku_msgs__msg__ButtonColorForm), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tku_msgs__msg__ButtonColorForm__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tku_msgs__msg__ButtonColorForm__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tku_msgs__msg__ButtonColorForm__Sequence__fini(tku_msgs__msg__ButtonColorForm__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tku_msgs__msg__ButtonColorForm__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tku_msgs__msg__ButtonColorForm__Sequence *
tku_msgs__msg__ButtonColorForm__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tku_msgs__msg__ButtonColorForm__Sequence * array = (tku_msgs__msg__ButtonColorForm__Sequence *)allocator.allocate(sizeof(tku_msgs__msg__ButtonColorForm__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tku_msgs__msg__ButtonColorForm__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tku_msgs__msg__ButtonColorForm__Sequence__destroy(tku_msgs__msg__ButtonColorForm__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tku_msgs__msg__ButtonColorForm__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tku_msgs__msg__ButtonColorForm__Sequence__are_equal(const tku_msgs__msg__ButtonColorForm__Sequence * lhs, const tku_msgs__msg__ButtonColorForm__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tku_msgs__msg__ButtonColorForm__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tku_msgs__msg__ButtonColorForm__Sequence__copy(
  const tku_msgs__msg__ButtonColorForm__Sequence * input,
  tku_msgs__msg__ButtonColorForm__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tku_msgs__msg__ButtonColorForm);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tku_msgs__msg__ButtonColorForm * data =
      (tku_msgs__msg__ButtonColorForm *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tku_msgs__msg__ButtonColorForm__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tku_msgs__msg__ButtonColorForm__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tku_msgs__msg__ButtonColorForm__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
